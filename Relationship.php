...
class RelationShip
{
	...
	private $_data = [
		...
		'f,xs,d&l' => ['姑表妹','表妹'],
		'f,xs,d&l,h' => ['姑表妹夫','表妹夫'],
		'f,xs,d,s' => ['表外甥','姑表甥男','表甥男','甥男'],
		'f,xs,d,d' => ['表外甥女','姑表甥女','表甥女','甥女'],
		'f,os' => ['姑妈','姑母'],
		'f,ls' => ['姑妈','姑姐'],
		//外家
		...
		'm,xb,d&l' => ['舅表妹','表妹'],
		'm,xb,d&l,h' => ['舅表妹夫','表妹夫'],
		'm,xb,d,s' => ['表外甥','舅表甥男','表甥男','甥男'],
		'm,xb,d,d' => ['表外甥女','舅表甥女','表甥女','甥女'],
		'm,ob' => ['大舅'],
		'm,ob,w' => ['大舅妈'],
		'm,lb' => ['小舅','舅父仔'],
		'm,lb,w' => ['小舅妈'],
		...
		'm,xs,d&l' => ['姨表妹','表妹'],
		'm,xs,d&l,h' => ['姨表妹夫','表妹夫'],
		'm,xs,d,s' => ['表外甥','姨表甥男','姨表甥男','甥男'],
		'm,xs,d,d' => ['表外甥女','姨表甥女','表甥女','甥女'],
		'm,os' => ['大姨','大姨妈'],
		'm,os,h' => ['大姨父','大姨丈'],
		'm,ls' => ['小姨','小姨妈','姨仔'],
		'm,ls,h' => ['小姨父','小姨丈'],
    ...
    // 分词解析
    private function getSelectors(string $str)
    {
        $str = mb_ereg_replace('/[二|三|四|五|六|七|八|九|十]{1,2}/', 'x', trim($str));
        $str = mb_ereg_replace('/我|家|^的+|的+$|\s+/', '', $str);
        $lists = explode('的', $str);

        $result = [];
        $match = true;
        while ( !empty($lists) ) {
            $name = array_shift($lists);
            $arr = [];
            $has = false;

            foreach ( $this->_data as $k=>$v ) {
                if ( in_array($name, $v, true) ) {
                    if ( $k || !empty($lists) ) $arr[] = $k;
                    $has = true;
                }
            }
            if ( !$has ) $match = false;
	...
	// 简化选择器
	private function selector2id($selector, $sex)
	{
		$result = [];
		$hash = [];
		if ( $sex < 0) {
			if ( strpos($selector, ',w') === 0 ) {
				$sex = 1;
			} elseif ( strpos($selector, ',h') === 0 ) {
				$sex = 0;
			}
		}
		if ( $sex > -1 ) $selector = ",{$sex}{$selector}";
		if ( preg_match('/,[w0],w|,[h1],h/', $selector) ) return [];

		$getId = function($selector) use (&$getId, &$hash, &$result) {
			$s = '';
			if ( !isset($hash[$selector]) ) {
				...
				do {
					...
					foreach ( $this->_filter as $v ) {
						$selector = preg_replace($v['exp'], $v['str'], $selector);
				...
				if ( $status ) {
					if ( preg_match('/,[w0],w|,[h1],h/', $selector) ) return [];
					$selector = preg_replace('/,[01]/', '', $selector);
	...
	private function getDataById($id)
	{
		...
		$getData = function($d) {
			$res = [];
			foreach ( $this->_data as $k=>$v ) {
				if ( preg_replace('/&[olx]/', '', $k) === $d ) $res[] = $v[0];
			}
			return $res;
		};

		if ( isset($this->_data[$id]) ) {
			$items[] = $this->_data[$id][0];
		} else {
			$items = $getData($id);
			if ( empty($items) ) {
				$id = preg_replace('/&[ol]/', '', $id);
				$items = $getData($id);
			}
			if ( empty($items) ) {
				$id = preg_replace('/[ol]/', 'x', $id);
				$items = $getData($id);
			}
			if ( empty($items) ) {
	...
	// 逆转id
	private function reverseId($id, $sex)
	{
		$hash = [
			'f' => ['d','s'],
			'm' => ['d','s'],
		...
		$age = '';
		if ( strpos($id, '&o') !== false ) $age = '$l';
		elseif ( strpos($id, '&l') !== false ) $age = '&o';

		if ( $id ) {
			$id = preg_replace('/&[ol]/', '', $id);
			$sex = $sex ? 1 : 0;
			$sid = preg_replace('/,[fhs]|,[olx]b/', ',1', ",{$sex},{$id}");
			$sid = preg_replace('/,[mwd]|,[olx]s/', ',0', $sid);
			$sid = substr($sid, 0, strrpos($sid, ','));
			$id_arr = array_reverse(explode(',', $id));
			$sid_arr = array_reverse(explode(',', $sid));
			$arr = [];
			for ( $i = 0; $i < count($id_arr); $i++ ) {
				$arr[] = $hash[$id_arr[$i]][$sid_arr[$i]];
			}
			return implode(',', $arr) . $age;
		}
		return '';
	}

	// 获取关系链条
	private function getChainById($id)
	{
		$arr = explode(',', $id);
		$items = [];
		foreach ( $arr as $v ) {
			$key = preg_replace('/&[ol]/', '', $v);
			$items[] = $this->_data[$key][0];
		}
		return implode('的', $items);
	}

	public function getRelation($text='', $sex=-1, $type='default', $reverse=false)
	{
		$selectors = $this->getSelectors($text);
		$result = [];
		foreach ( $selectors as $selector ) {
			$ids = $this->selector2id($selector, $sex);
			foreach ( $ids as $id ) {
				if ( $type == 'chain' ) {
					$item = $this->getChainById($id);
					if ( $item ) $result[] = $item;
				} else {
					if ( $reverse ) $id = $this->reverseId($id, $sex);
					$items = $this->getDataById($id);
					if ( !empty($items) ) {
						$result = array_merge($result, $items);
					} elseif ( strpos($id, 'w') === 0 || strpos($id, 'h') === 0 ) {
						$items = $this->getDataById(substr($id, 2));
						if ( !empty($items) ) $result = array_merge($result, $items);
					}
				}
			}
		}
...
$r = $R->getRelation('爸爸的妈妈的孙子', 0);  // 第二个参数为”我“的性别, 0=女, 1=男, -1=未知
$r = $R->getRelation('爸爸的妈妈的孙子', 1);
$r = $R->getRelation('爸爸的妈妈的孙子', 1, 'default', true);  // 逆向称呼, ”爸爸的妈妈的孙子“ 称呼 ”我“ 什么
$r = $R->getRelation('爸爸的妈妈的孙子', 1, 'chain');   // 关系链
